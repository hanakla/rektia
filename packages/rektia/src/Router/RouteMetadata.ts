...
export const rectiaRouteAllMethods = Object.freeze([
  "GET",
  "POST",
  "PUT",
  "PATCH",
...
export default class RelationMetadata {
  ...
  private static _initialMetadata(): IRouteMetadata {
    return {
      methods: new Set(),
      controllerName: null,
      extraMatchPattern: null
    };
  }

  public static setControllerName(
    actionMethod: Action,
    controllerName: string
  ) {
    const meta: IRouteMetadata =
      this.getFor(actionMethod) || this._initialMetadata();
  ...
  public static getControllerNameFromFirstRegisteredAction(
    actionMethods: Action[]
  ): string | null {
    const registeredAction = actionMethods.find(action =>
      this._metadata.has(action)
    );
  ...
  public static addAllowedMethod(
    actionMethod: Action,
    methods: MethodTypes | MethodTypes[]
  ) {
