...
export default class TypeGenerator {
    ...
    public async generateTypeDefinition()
    {
        ...
        const columnDefs: [string, {[p: string]: Knex.ColumnInfo}][] = await Promise.all(tables.map(async tableName => [tableName, await knex(tableName).columnInfo()])) as any

        const indent = (num: number = 1) => '    '.repeat(num)

        const interfaceDefs = columnDefs.map(([tableName, columnDef]) => {
            const interfaceName = DBTypeGeneratorHelper.modelClassNameFromTableName(tableName)
            ...
            return `${indent(2)}interface ${interfaceName} {\n${columnTypes}\n${indent(2)}}`
        }).join('\n\n')

        fs.writeFileSync(path.join(this._options.appDir, 'app/app.d.ts')
